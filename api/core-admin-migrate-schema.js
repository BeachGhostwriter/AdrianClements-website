...
const MIGRATION_SQL = `-- CreateEnum
...
export default async function handler(req, res) {
  ...
  if (!process.env.MIGRATION_TOKEN2 || token !== process.env.MIGRATION_TOKEN2) {
    return res.status(401).json({ error: 'Not authorized.' });
  }
  try {
    const pool = getPool();
    await pool.query(MIGRATION_SQL);
    const check = await pool.query("SELECT table_name FROM information_schema.tables WHERE table_schema='public' ORDER BY table_name");
    return res.status(200).json({ ok: true, tables: check.rows.map(r => r.table_name) });
  } catch (err) {
